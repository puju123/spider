...
import com.pujun.spider.tool.URLFilter;
...
//  }
   ...
   * nekohtml).
   */
	public void getOutlinks(String html, ArrayList<String> outlinks) {
				...
				while (iterator.hasNext()) {
					...
					if (params.attrName.equalsIgnoreCase(attrName)) {
						target = attr.getValue();
							...
							&& "nofollow".equalsIgnoreCase(attr.getValue())) {
						noFollow = true;
					} else if ("method".equalsIgnoreCase(attrName)
							&& "post".equalsIgnoreCase(attr.getValue())) {
						post = true;
					}
				}
				if (target != null && !noFollow && !post&& URLFilter.filt(target)) {
					outlinks.add(target);
				}
				// }
